...
def get_equipment_records(
    ...
    if current_user.get("role") == "manager":
        ...
        print(f"get_equipment_records - manager role, user_team_id from token: {user_team_id}, team_id param: {team_id}, current_user: {current_user}")
        ...
        if team_id:
            if team_id != user_team_id:
                print(f"get_equipment_records - WARNING: manager tried to access different team! user_team_id: {user_team_id}, requested team_id: {team_id}")
                raise HTTPException(
                    status_code=403,
